...
GitLogView.prototype.fill_content = function() {
    var create_main_row = function(log) {
        ...
        var refsString = (log.refs || [])
          .map(ref => {
            var headPattern = /^HEAD -> refs\/heads\/(.*)/;
            var remotePattern = /^refs\/remotes\/(.*)/;
            var localPattern = /^refs\/heads\/(.*)/;

            var headMatches = ref.match(headPattern);
            if (headMatches) {
              return 'HEAD -> ' + headMatches[1];
            }

            var remoteMatches = ref.match(remotePattern);
            if (remoteMatches) {
              return remoteMatches[1];
            }

            var localMatches = ref.match(localPattern);
            if (localMatches) {
          ...
          .map(ref => `<span class='ref ${log.position.color}'>${ref}</span>`).join('');
        html += '<td><p>' + refsString + log.message.split('\n')[0] + '</p></td>';
